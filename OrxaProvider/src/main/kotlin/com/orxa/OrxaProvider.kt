...
class OrxaProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val response = app.get("$mainUrl/api/movie?id=${id.encodeUrl()}&lang=ar")

        if (id.startsWith("vidsrc-tv:")) {
            val details = response.parsed<SeriesDetailResponse>().details
            val episodes = details.seasons.flatMap { seasonInfo ->
                (1..seasonInfo.episodeCount).map { episodeNumber ->
                    newEpisode("${id}|${seasonInfo.seasonNumber}|$episodeNumber") {
                        this.name = "الحلقة $episodeNumber"
                        this.season = seasonInfo.seasonNumber
